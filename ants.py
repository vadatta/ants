...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        counter = 0
        location = self.place
        while location.is_hive == False:
            if location.bees != [] and self.min_range <= counter <= self.max_range:
...
# END Problem 12
    ...
    def __init__(self, gamestate, health = 1):
         # BEGIN Problem 12
        super().__init__(health)
        if gamestate.queen_placed == False:
    ...
    def action(self, gamestate):
        ...
        while current_place.exit != None:
            ...
            tunneled_ant = next_place.ant
            if tunneled_ant != None:
                if tunneled_ant.is_container == True and tunneled_ant.ant_contained != None:
                    Ant.double(tunneled_ant)
                    Ant.double(tunneled_ant.ant_contained)
                else:
                    Ant.double(tunneled_ant)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.scared_turns > 0 and self.slowed_turns > 0:
                ...
                if not self.place.exit.is_hive:
                    self.move_to(self.place.entrance)
                self.scared_turns -= 1
            else:
                self.move_to(destination)
